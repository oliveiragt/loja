...
//-->
  ...
  <body>
  <div id="conteudo">
  <center><h2>Edição de Usuários</h2></center>
  <hr><?php if($_SESSION['cargo']=="admin"){
	  include("conexao.php");
	  $id=$_GET['id'];
	  $busca=mysql_query("SELECT * FROM usuarios WHERE idusuario='$id'");
	  while($exibe=mysql_fetch_assoc($busca)){
	  ?>
  <table id="tblcadastro1">
  <form  id="cadastraprod" action="editausu.php" method="post">
  <tr>
  <td>
  <input type="text" name="id" style="display:none" value="<?php echo $id; ?>">
  Nome
  </td><td><input class="func" type="text" name="nome" value="<?php echo $exibe['nome']; ?>"></td></tr>
  <tr><td>
  Senha:</td><td> <input class="func"type="text" name="senha" value="<?php echo $exibe['senha']; ?>"></td></tr>
  <tr><td>
  Login:</td><td><input class="func" type="text" name="login" value="<?php echo $exibe['login']; ?>"></td></tr>
  <tr>
  <tr><td>
  Cargo:</td><td><input class="func" type="text" name="cargo" value="<?php echo $exibe['cargo']; ?>"></td></tr>
  <tr>
  ...
  <a href="editafunc.php">Voltar a página anterior</a>
  </div>
  <div id="fundo">
